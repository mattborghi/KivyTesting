...
class ToolButton(ToggleButton):
	def on_touch_down(self, touch):
		...
		# Is the button pressed and the event happened in the DrawingSpace instace(ds)?
		if self.state == 'down' and ds.collide_point(touch.x, touch.y):
			# Before sending the coordinates to draw(), we translate the absolute coordinates to relative coordinates (appropiated for the drawing space)
			(x,y) = ds.to_widget(touch.x, touch.y)
			# Draw the respective shapes according to the derived classes
			self.draw(ds, x, y)
			return True
...
class ToolStickman(ToolButton):
	def draw(self, ds, x, y):
		...
		# Use the translated coordinates to center the Stickman
		sm.center = (x,y)
		# Add it to the DrawingSpace 
		ds.add_widget(sm)
